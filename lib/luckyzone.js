...
function getPatternForBusinessDateKey(businessDateKey, settings = {}) {
  const anchorDate = settings.anchorDate || DEFAULT_ANCHOR_DATE;
  const anchorPattern = Number(settings.anchorPattern || DEFAULT_ANCHOR_PATTERN);
  const cycleDays = Number(settings.cycleDays || DEFAULT_CYCLE_DAYS);
  const daysSinceAnchor = diffDateKeys(businessDateKey, anchorDate);
  const cycleIndex = Math.floor(daysSinceAnchor / cycleDays);
  return ((anchorPattern - 1 + cycleIndex) % 3 + 3) % 3 + 1;
}

function getScheduleForBusinessDateKey(businessDateKey, settings = {}) {
  const [year, month, day] = businessDateKey.split("-").map(Number);
  const pattern = getPatternForBusinessDateKey(businessDateKey, settings);
...
function getNextLuckyZoneSchedule(date = new Date(), settings = {}) {
  // /luckyzone next always means the next calendar day in Jakarta,
  // regardless of whether the current 08:00 reset has happened yet.
  const tomorrowKey = addDaysToDateKey(jakartaDateKey(date), 1);
  return getScheduleForBusinessDateKey(tomorrowKey, settings);
}
...
function getTodaySchedule(date = new Date(), settings = {}) {
  return getScheduleForBusinessDateKey(getLuckyZoneBusinessDateKey(date), settings);
}
...
function buildLuckyZoneEmbed(now = new Date(), settings = {}, guildId = null) {
  ...
  const dateText = lang === "en"
    ? new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Jakarta", weekday: "long", day: "numeric", month: "long", year: "numeric" }).format(new Date(`${schedule.businessDateKey}T00:00:00Z`))
    : formatIndonesianDate(new Date(`${schedule.businessDateKey}T00:00:00Z`));

  return new EmbedBuilder()
    .setColor(0x00f0a8)
    .setTitle(`🍀 ${t(lang, "lz_daily_rotation")}`)
    .setDescription(
      `📅 **${dateText}**` +
      `\n🎯 **Pattern ${schedule.pattern}**` +
      `\n\n✨ **${t(lang, "lz_active_today")}**` +
      `\n🔄 ${t(lang, "lz_reset_daily")}`
    )
    .addFields(
      {
        name: `🟢 Lucky Zone I  •  ${t(lang, "lz_map_active")}`,
        value: `> 🗺️ **${schedule.luckyZone1}**`,
        inline: false
      },
      {
        name: `🔵 Lucky Zone II  •  ${t(lang, "lz_map_active")}`,
        value: `> 🗺️ **${schedule.luckyZone2}**`,
        inline: false
      },
      {
        name: `⏰ ${t(lang, "lz_reset_lz")}`,
        value: `> 🕗 **<t:${nextDailyUnix}:R>**\n> 📅 <t:${nextDailyUnix}:F>`,
        inline: false
      },
      {
        name: `🔁 ${t(lang, "lz_reset_pattern")}`,
        value: `> 🎯 Pattern **${nextPattern}**\n> 🕗 **<t:${nextPatternUnix}:R>**\n> 📅 <t:${nextPatternUnix}:F>`,
        inline: false
      }
    )
    .setFooter({ text: t(lang, "footer_luckyzone") });
}

function buildLuckyZoneNextEmbed(now = new Date(), settings = {}, guildId = null) {
  const lang = guildLanguage(guildId);
  const schedule = getNextLuckyZoneSchedule(now, settings);
  const targetReset = getJakartaDateTimeFor8AM(schedule.businessDateKey);
  const targetUnix = toDiscordTimestamp(targetReset);
  const dateText = lang === "en"
    ? new Intl.DateTimeFormat("en-US", {
        ...
        month: "long",
        year: "numeric"
      }).format(new Date(`${schedule.businessDateKey}T00:00:00Z`))
    : formatIndonesianDate(new Date(`${schedule.businessDateKey}T00:00:00Z`));

  return new EmbedBuilder()
    .setColor(0x00f0a8)
    .setTitle(`🍀 ${t(lang, "lz_next_title")}`)
    .setDescription(
      `📅 **${dateText}**` +
      `\n🎯 **Pattern ${schedule.pattern}**` +
      `\n\n✨ **${t(lang, "lz_next_active")}**`
    )
    .addFields(
      {
        name: `🟢 Lucky Zone I  •  ${t(lang, "lz_map_active")}`,
      ...
      {
        name: `⏰ ${t(lang, "lz_next_reset")}`,
        value: `> 🕗 **<t:${targetUnix}:R>**\n> 📅 <t:${targetUnix}:F>`,
        inline: false
      }
    )
    .setFooter({ text: t(lang, "footer_luckyzone") });
...
module.exports = {
  ...
  getPatternForDate,
  getPatternForBusinessDateKey,
  getScheduleForBusinessDateKey,
  getNextLuckyZoneSchedule,
  getTodaySchedule,
  getNextDailyReset,
  getNextPatternReset,
  buildLuckyZoneNextEmbed,
